...
class CADAnalyzer:
    ...
    def convert_dwg_to_dxf(self, dwg_file_path: str) -> str:
        """DWG dosyasını DXF'e çevirir"""
        try:
            # Çıkış dosya yolu
            dxf_file_path = dwg_file_path.replace('.dwg', '.dxf')
            
            # Method 1: ODA File Converter kullan (eğer kuruluysa)
            if self.check_oda_converter():
                return self.convert_with_oda(dwg_file_path, dxf_file_path)
            
            # Method 2: dwg2dxf kütüphanesi kullan
            elif self.check_dwg2dxf():
                return self.convert_with_dwg2dxf(dwg_file_path, dxf_file_path)
            
            # Method 3: LibreCAD kullan (eğer kuruluysa)
            elif self.check_librecad():
                return self.convert_with_librecad(dwg_file_path, dxf_file_path)
            
            else:
                raise Exception("DWG dönüştürücü bulunamadı. Lütfen dosyayı manuel olarak DXF'e çevirin.")
                
        except Exception as e:
            raise Exception(f"DWG dönüştürme hatası: {str(e)}")
    
    def check_oda_converter(self) -> bool:
        """ODA File Converter kurulu mu kontrol eder"""
        try:
            # Windows
            if platform.system() == "Windows":
                result = subprocess.run(['where', 'ODAFileConverter'], 
                                      capture_output=True, text=True)
                return result.returncode == 0
            # Linux/Mac
            else:
                result = subprocess.run(['which', 'ODAFileConverter'], 
                                      capture_output=True, text=True)
                return result.returncode == 0
        except:
            return False
    
    def check_dwg2dxf(self) -> bool:
        """dwg2dxf kütüphanesi var mı kontrol eder"""
        try:
            import dwg2dxf
            return True
        except ImportError:
    ...
    def check_librecad(self) -> bool:
        """LibreCAD kurulu mu kontrol eder"""
        try:
            if platform.system() == "Windows":
                result = subprocess.run(['where', 'librecad'], 
                                      capture_output=True, text=True)
                return result.returncode == 0
            else:
                result = subprocess.run(['which', 'librecad'], 
                                      capture_output=True, text=True)
                return result.returncode == 0
        except:
            return False
    
    def convert_with_oda(self, input_path: str, output_path: str) -> str:
        """ODA File Converter ile dönüştür"""
        try:
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                ...
                raise Exception(f"ODA dönüştürme hatası: {result.stderr}")
                
        except subprocess.TimeoutExpired:
            raise Exception("Dönüştürme zaman aşımına uğradı")
        except Exception as e:
            raise Exception(f"ODA dönüştürme hatası: {str(e)}")
    
    def convert_with_dwg2dxf(self, input_path: str, output_path: str) -> str:
        """dwg2dxf kütüphanesi ile dönüştür"""
        try:
            import dwg2dxf
            dwg2dxf.convert(input_path, output_path)
            
            if os.path.exists(output_path):
                return output_path
            else:
                raise Exception("DXF dosyası oluşturulamadı")
                
        except Exception as e:
            raise Exception(f"dwg2dxf dönüştürme hatası: {str(e)}")
    
    def convert_with_librecad(self, input_path: str, output_path: str) -> str:
        """LibreCAD ile dönüştür"""
        try:
            cmd = ['librecad', '-o', output_path, input_path]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
            
            if os.path.exists(output_path):
                return output_path
            else:
                raise Exception("LibreCAD dönüştürme başarısız")
                
        except Exception as e:
            raise Exception(f"LibreCAD dönüştürme hatası: {str(e)}")
    
    def analyze_cad_file(self, file_content: bytes, filename: str) -> Dict:
        """CAD dosyasını (DWG veya DXF) analiz eder"""
        ...
        try:
            ...
            # DWG dosyası ise DXF'e çevir
            if file_extension == 'dwg':
                st.info("🔄 DWG dosyası DXF'e çevriliyor...")
                dxf_file_path = self.convert_dwg_to_dxf(temp_file_path)
                analysis_file = dxf_file_path
            else:
                analysis_file = temp_file_path
            
            # DXF dosyasını analiz et
            result = self.analyze_dxf_file_from_path(analysis_file)
            
            # Geçici dosyaları temizle
            try:
                os.unlink(temp_file_path)
                if file_extension == 'dwg' and os.path.exists(dxf_file_path):
    ...
    def analyze_dxf_file_from_path(self, file_path: str) -> Dict:
        """Dosya yolundan DXF dosyasını analiz eder"""
        try:
            # DXF dosyasını oku
    ...
    def analyze_dxf_file(self, file_content: bytes) -> Dict:
        """DXF dosyasını analiz eder (geriye uyumluluk için)"""
        try:
            # Geçici dosya oluştur
            ...
            result = self.analyze_dxf_file_from_path(temp_file_path)
...
def main():
    ...
    # DWG dönüştürücü durumu kontrol et
    analyzer = CADAnalyzer()
    has_converter = (analyzer.check_oda_converter() or 
                    analyzer.check_dwg2dxf() or 
                    analyzer.check_librecad())
    
    # Desteklenen formatlar
    if has_converter:
        file_types = ['dxf', 'dwg']
        help_text = "DXF ve DWG formatları desteklenmektedir"
        st.sidebar.success("✅ DWG dönüştürücü mevcut")
    else:
        file_types = ['dxf']
        help_text = "Sadece DXF formatı desteklenmektedir. DWG desteği için dönüştürücü gerekli."
        st.sidebar.warning("⚠️ DWG dönüştürücü bulunamadı")
        
        # Kurulum talimatları
        with st.sidebar.expander("🔧 DWG Desteği İçin Kurulum"):
            st.markdown("""
            **Seçenek 1 - ODA File Converter (Önerilen):**
            1. [ODA Member Site](https://www.opendesign.com/guestfiles/oda_file_converter) 'den indirin
            2. Kurun ve PATH'e ekleyin
            
            **Seçenek 2 - Python kütüphanesi:**
            ```bash
            pip install dwg2dxf
            ```
            
            **Seçenek 3 - LibreCAD:**
            - Ubuntu: `sudo apt install librecad`
            - Windows: LibreCAD resmi sitesinden indirin
            """)
    ...
    if uploaded_file is not None:
      ...
      DWG dosyası ve dönüştürücü yoksa uyarı
        if file_extension == 'dwg' and not has_converter:
            st.error("❌ DWG dosyası yüklendi ancak dönüştürücü bulunamadı! Lütfen dosyayı DXF formatında yükleyin veya dönüştürücü kurun.")
            return
        ...
        # Dosyayı analiz et
        with st.spinner(f'{file_extension.upper()} dosyası analiz ediliyor...'):
            file_content = uploaded_file.read()
            result = analyzer.analyze_cad_file(file_content, uploaded_file.name)
